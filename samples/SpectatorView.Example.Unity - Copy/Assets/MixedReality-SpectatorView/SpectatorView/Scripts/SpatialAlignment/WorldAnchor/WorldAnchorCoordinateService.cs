...
namespace Microsoft.MixedReality.SpectatorView
{
    ...
    public class WorldAnchorCoordinateService : SpatialCoordinateServiceUnityBase<string>
    {
...
#if UNITY_WSA
        private Task<ARAnchorManager> worldAnchorStoreTask;
#endif
        ...
        {
#if UNITY_WSA
            ARAnchorManager store = await GetWorldAnchorStoreAsync();

            //foreach (string knownId in store.GetAllIds())
            //{
            //    GameObject anchorGameObject = new GameObject(knownId);
            //    ARAnchor anchor = store.Load(knownId, anchorGameObject);
        ...
        public override async Task<bool> TryDeleteCoordinateAsync(string key, CancellationToken cancellationToken)
        {
            ...
            if (TryGetKnownCoordinate(key, out ISpatialCoordinate coordinate))
            {
                ...
                ARAnchorManager anchorStore = await GetWorldAnchorStoreAsync();
...
#if UNITY_WSA
        //private async void Anchor_OnTrackingChanged(ARAnchor worldAnchor, bool located)
        //{
        //    if (located)
        //    {
        ...
        private Task<ARAnchorManager> GetWorldAnchorStoreAsync()
        {
        ...
        private Task<ARAnchorManager> InitializeWorldAnchorStoreAsync()
        {
            TaskCompletionSource<ARAnchorManager> taskSource = new TaskCompletionSource<ARAnchorManager>();
            //ARAnchorManager.GetAsync(store => taskSource.SetResult(store));
        ...
        private class WorldAnchorSpatialCoordinate : SpatialCoordinateUnityBase<string>
        {
            ...
            public WorldAnchorSpatialCoordinate(string id, ARAnchor worldAnchor)
                : base(id)
            {
                this.worldAnchor = worldAnchor;
                ARSession.stateChanged += WorldAnchor_OnTrackingChanged;
            }
            ...
            private void WorldAnchor_OnTrackingChanged(ARSessionStateChangedEventArgs args)
            {
                OnStateChanged();
            }
